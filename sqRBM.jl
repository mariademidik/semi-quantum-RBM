...
function get_pv(model::String, H::Dict, num_visible::Int64, num_hidden::Int64)
    ...
    paulis = ["Z", "X", "Y"]
    bitstrings =
        [reverse(digits(x, base = 2, pad = num_visible)) for x = 0:(2^num_visible-1)]
...
function train(
    ...
    loss_path =
        path * @sprintf("%s_%dv_%dh_%d_loss.csv", model, num_visible, num_hidden, runID)
    grads_path =
        path * @sprintf("%s_%dv_%dh_%d_grads.csv", model, num_visible, num_hidden, runID)
    coef_path =
        path * @sprintf("%s_%dv_%dh_%d_coef.csv", model, num_visible, num_hidden, runID)
    ...
    coef_hist = Dict(op .=> [Vector{Float64}() for _ = 1:num_terms])
    ...
    for i = 1:num_iter
        local step_time = @elapsed begin # time iteration step
            ...
            # compute gradients
            grads =
                get_grad(model, H, num_visible, num_hidden, pv, p_data, p_tilde, Z, ɸ)
